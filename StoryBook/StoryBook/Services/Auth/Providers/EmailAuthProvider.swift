...
// MARK: - メール認証プロバイダー
class EmailAuthProvider: ObservableObject, AuthProviderProtocol {
    ...
    func login(completion: @escaping (AuthResult) -> Void) {
        ...
        // Auth0のUniversal Loginでメール認証を指定
        Auth0
        ...
        completion(AuthResult(success: false, provider: .email, error: NSError(domain: "Auth0", code: -1, userInfo: [NSLocalizedDescriptionKey: "Auth0モジュールが利用できません"])))
        #endif
    }
            ...
            .clearSession(federated: false) { [weak self] result in
                DispatchQueue.main.async {
                    ...
                    case .failure(let error):
                        self?.errorMessage = "メールログアウトに失敗しました: \(error.localizedDescription)"
                        print("❌ メールログアウトエラー: \(error)")
                        completion(false)
                    }
                }
            }
    ...
    private func handleAuthResult(_ result: Auth0.WebAuthResult<Auth0.Credentials>, completion: @escaping (AuthResult) -> Void) {
        ...
        case .failure(let error):
            isLoggedIn = false
            errorMessage = "メールログインに失敗しました: \(error)"
            print("❌ メールログインエラー詳細: \(error)")
